    ...
    <form action="" class="cont_barr_busque">
  	...
  	<div class="contenedor">
        ...
        <?php
            ...
            $consulta1=mysqli_query($con,"SELECT * from cabecera_examen E inner join paciente P on P.id_paciente=E.id_paciente inner join cabecera_resultado CR on CR.cabecera_exa_id=E.cabecera_exa_id inner join tipo_formato TF on TF.formato_id=CR.cabecera_tipo_formato WHERE E.estado_exa_id ='1' and E.id_paciente='$id' and fecha_ingreso_exa>= now()-interval 3 month ");
            while($row1=mysqli_fetch_array($consulta1)){
                $tipoPago = $row1['tipo_pago'];
                $precio = $row1['precio_examen'];
                $abono = $row1['abono'];
                if($tipoPago==1 and ($abono==0 or $abono=="0.00" or $abono=="0.0")){
                    $mnsj="pagado";
                }elseif($tipoPago==2 and $precio==$abono){
                    $mnsj="pagado";
                }else{
                    $mnsj="abonado";
                }
