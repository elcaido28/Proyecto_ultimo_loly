...
$y=$pdf->GetY();
  ...
  if ($nrow5<1) {
    $pdf->SetXY(90,$y);
    $pdf->Cell(70,10,utf8_decode(strtoupper($row2['detalle_resultado_tipoexa'])),0,0,'L');
  }else{
    $pdf->SetXY(55,$y);
    $pdf->SetFont('arial','B',12);
    $pdf->Cell(70,10,utf8_decode(strtoupper($row2['detalle_resultado_tipoexa'].":")),0,0,'L');
  ...
  if ($cuenta>1) {
    ...
    $pdf->SetFont('arial','',12);
    $pdf->Cell(50,10,utf8_decode(strtoupper($row2['detalle_resultado_resul'])),0,1,'L');
  }
  $nrow5++;
}
$y=$pdf->GetY();
...
$row50=mysqli_fetch_array($consulta33);
if ($row50['detalle_resultado_resul']!='') {
    $pdf->SetFont('arial','BU',12);
    $pdf->Cell(100,10,'EN FRESCO',0,1,'C');
}else{
    // $pdf->SetFont('arial','BU',12);
...
$pdf->SetXY(50,$y);
    ...
    if($row3['detalle_resultado_tipoexa']=="Null"){
        ...
        $pdf->Cell(80,6,utf8_decode(strtoupper($row3['detalle_resultado_tipoexa'].":")),0,0,'L');
    }
    ...
    if($row3['detalle_resultado_resul']!=''){
        ...
        if ($cuenta>1) {
            ...
            $pdf->Cell(50,6,utf8_decode($row3['detalle_resultado_resul']),0,1,'L');
        }
    }else{}
}
$y=$pdf->GetY();
...
$y=$pdf->GetY();
 $pdf->SetXY(55,$y);
  ...
  if($row4['detalle_resultado_tipoexa']=="Null"){
        ...
        $pdf->Cell(70,6,utf8_decode(strtoupper($row4['detalle_resultado_tipoexa'].":")),0,0,'L');
    }
  ...
  if ($cuenta>1) {
    ...
    $pdf->Cell(50,6,utf8_decode($row4['detalle_resultado_resul']),0,1,'L');
  }
}
$y=$pdf->GetY();
